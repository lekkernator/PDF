...
public class Player2AnimationController : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Slash) && !isPunching)
        {
            animator.Play("New State");
            Debug.Log("punching");
            StartCoroutine(Punch());
        }
        else if (animator.GetBool("IsGrounded") && !isPunching && !punchRecovery && ( Input.GetKey(KeyCode.LeftArrow) || Input.GetKey(KeyCode.RightArrow))) 
        {
        ...
        else if (!isPunching && !punchRecovery && (Input.GetKey(KeyCode.RightArrow) || Input.GetKey(KeyCode.LeftArrow) || Input.GetKey(KeyCode.UpArrow) || Input.GetKey(KeyCode.DownArrow) || Input.GetKey(KeyCode.Slash)))
        {
            walkTime = 0f;
            timeSinceLastMove = 0;
            animator.Play("New State");
