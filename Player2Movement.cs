...
public class Player2Movement : MonoBehaviour
{
    ...
    void Update()
    {
        if (Time.timeScale != 0)
        {
            ...
            if ((!punching && !punchRecovery) || ((punching || punchRecovery) && !isGrounded))
            {
                ...
                if (Input.GetKeyDown(KeyCode.UpArrow) && rb.constraints != (RigidbodyConstraints2D.FreezePositionX | RigidbodyConstraints2D.FreezePositionY))
                {
    ...
    private IEnumerator punch()
    {
        ...
        Debug.Log("Collider Should be enabled: " + attackCollider.enabled);
