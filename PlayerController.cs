...
public class PlayerController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        damageReceiver = GetColliderFromTaggedChild("Player1SubLayer") as BoxCollider2D; // Get the player's damage receiver collider
    ...
    void Update()
    {
        ...
        if (!respawning && Input.GetKeyDown(KeyCode.R) && energyBallSpawnTimer >= EnergyBallSpawnCooldown)
        {
            ShootEnergyBall();
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if ( collision.gameObject.tag != "Platform" && collision.gameObject.tag != "Ground" && collision.gameObject.tag != "Player2SubLayer")
        {
        ...
        if ((collision.gameObject.tag == "Player2SubLayer" || collision.gameObject.tag == "Player"))
        {
